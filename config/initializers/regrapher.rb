...
ActiveSupport::Notifications.subscribe 'process_action.action_controller' do |name, started, finished, unique_id, data|
  ...
  Regrapher.client.gauge("#{name}.view_runtime", data[:view_runtime].to_f)
  Regrapher.client.gauge("#{name}.db_runtime", data[:db_runtime].to_f)
  Regrapher.client.gauge("#{name}.runtime", (finished - started) * 1000.0)
